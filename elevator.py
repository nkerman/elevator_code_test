...
def readable_time_delta(num_seconds: int):
    ...
    if num_seconds < 60:  # seconds range
        ...
        # use int to round *down* to nearest int
        m_component = int(num_seconds / 60)
...
class Elevator:
    ...
    def travel_through_floor_list(self, floor_list: list, verbosity: int = 2):
        ...
        assert check_all_ints(
            floor_list), "All values in the floor list must be ints or int-valued floats. "
        ...
        assert len(
            floor_list), "The input list must contain some floors which the elevator is not currently on."
        ...
        assert max(
            self.floor_list) <= self.building_height, "The elevator may never go higher than the top floor."
        # Set final floor
        self.final_floor = floor_list[-1]
        for floor in floor_list:
            self.next_floor = floor
            # add distance traveled
            self.odometer += abs(self.next_floor - self.current_floor)
            if self.next_floor:
