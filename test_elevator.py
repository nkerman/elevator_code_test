...
def test_readable_time_delta():
    ...
    assert readable_time_delta(
        time_in_sec) == "355 hours 14 minutes 11.5 seconds", "Incorrect time conversion"
    # Now for 0 time
    assert readable_time_delta(0) == "0.0 seconds", "Incorrect time conversion"
    # negative time
    assert readable_time_delta(
        -10000) == "-10000.0 seconds", "Incorrect time conversion"
# %%


def test_instantiate_Elevator():
    el = Elevator(10, 11, name="The Great Glass Elevator",
...
def test_dist_traveled_and_repeat_floors():
    ...
    assert el.floor_history == [
        9, 3, 9, 10, 1, 2, 10, 4], f"Floor history is unexpected: {el.floor_history}"
    ...
    assert el2.floor_history == [
        0, 1, 0], f"Floor history is unexpected: {el.floor_history}"
...
def test_time_elapsed():
    ...
    el.time_elapsed == 21 * \
        10, "After traveling 21 floors at 0.1 floor/sec, 210 sec should have elapsed."
    el.travel_through_floor_list([0, 5, 4, 33], verbosity=0)
    el.time_elapsed == 57 * \
        10, "After traveling 21 then an additional 36 floors at 0.1 floor/sec, 570 sec should have elapsed."
# %%
...
def test_negative_floors():
    ...
    assert el.floor_history == [
        0, -100, 100, -1000], "History not correctly recorded for negative floors."
    assert el.odometer == 1400, "Distance traveled not correctly recorded for negative floors."
    assert el.time_elapsed == 1400 * \
        10, "Time elapsed in transit not correctly recorded for negative floors."
# %%
...
def test_check_all_ints():
    ...
    with pytest.raises(AssertionError):
        ...
        assert check_all_ints(
            [1, 3, 3.1]), "Should fail to find only integers."
    # Succeeds if we give it only int-convertable floors
    assert check_all_ints(
        [1, 3, 3.0, ]), "Should succeed to find only integers, even when 3.0 is technically a float"
# %%
